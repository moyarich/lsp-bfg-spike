...
WITH (appendonly=true, orientation=parquet, pagesize=1048576, rowgroupsize=8388608, compresstype=snappy)
DISTRIBUTED BY (Order_ID)
PARTITION BY RANGE (order_datetime) (
  START (:DATA_START) END (:MONTHLY_END) EVERY (interval '1 month') WITH (appendonly=true, orientation=parquet, pagesize=1048576, rowgroupsize=8388608, compresstype=snappy),
  START (:MONTHLY_END) END (:PRELOAD_END) EVERY (interval '1 week') WITH (appendonly=true, orientation=parquet, pagesize=1048576, rowgroupsize=8388608, compresstype=snappy),
  PARTITION today START (:PRELOAD_END) END (:DATA_END) INCLUSIVE WITH (appendonly=true, orientation=parquet, pagesize=1048576, rowgroupsize=8388608, compresstype=snappy),
