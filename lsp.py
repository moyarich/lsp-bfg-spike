...
if __name__ == '__main__':
    ...
    if add_database and start_flag:
        ...
        if report_num > 0:
            ...
            from hst.f_generate_test_report_summary(%d, %d, 'PHD 2.2 build 59', 'HAWQ 2.0.0.0 build 12988 GVA ORCA OFF') where wl_name not like '%s' order by action_type, tr_id, s_id;" % (start_run_id, tr_id, '%' + 'RWITHD' + '%')
            ...
            sql = "select wl_name, action_type,overral_test_result,  improvenum, passnum, failurenum, skipnum, errornum, actual_total_execution_time,baseline_total_execution_time,deviation \
            from hst.f_generate_test_report_summary(%d, %d, 'PHD 2.2 build 59', 'HAWQ 1.2.1.2 build 11946 GVA ORCA OFF') where wl_name not like '%s' order by action_type, tr_id, s_id;" % (start_run_id, tr_id, '%' + 'RWITHD' + '%')
